...
if ($_SERVER['REQUEST_METHOD'] == "POST" && !empty(trim($_POST['pergunta']))) {
    ...
    # É iniciado o curl, responsável por baixar as informações fornecidas pela API
    $ch = curl_init($apiUrl);
    # Esse array se refere as opções do curl, nesse caso: 
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        # O curl fará a requisição no formato post, por motivos de segurança
        CURLOPT_POST => true,
    ...
    try {
        ...
        # a variavel $stmt representa statement, onde é feita a conexão com o banco de dados
